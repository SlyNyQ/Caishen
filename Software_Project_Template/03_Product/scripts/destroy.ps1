...
if (-not $env:TF_STATE_BUCKET) {
    throw "TF_STATE_BUCKET must be set before running destroy.ps1."
}

if (-not $env:TF_LOCK_TABLE) {
    throw "TF_LOCK_TABLE must be set before running destroy.ps1."
}
...
if (-not $env:TF_VAR_ecr_repository_url) {
    if ($env:ECR_REPOSITORY_URL) {
        ...
        throw "TF_VAR_ecr_repository_url or ECR_REPOSITORY_URL must be set before running destroy.ps1."
    }
}

if (-not $env:TF_VAR_app_name) {
    $env:TF_VAR_app_name = "caishen"
}
...
$env:TF_VAR_image_tag = "destroy-local"
